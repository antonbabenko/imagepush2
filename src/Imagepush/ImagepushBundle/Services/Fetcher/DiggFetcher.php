...
class DiggFetcher extends AbstractFetcher implements FetcherInterface
{
    ...
    public function isWorthToSave($item)
    {
        ...
        $result = (
            isset($item->diggs) &&
            $item->diggs >= $this->getParameter("min_diggs", 1) &&
        ...
        if ($image instanceof Image && time() < $image->getTimestamp()->sec + $minDelay) {
            ...
            if ($status === true) {
                ...
                if ($this->savedCounter == 0) {
                    ...
                    $this->output[] = sprintf("[DiggFetcher] %s: %d of %d items has been saved. Recent source date was on %s", date(DATE_RSS), $this->savedCounter, $this->fetchedCounter, date(DATE_RSS, $this->recentSourceDate));
                }
            } else {
